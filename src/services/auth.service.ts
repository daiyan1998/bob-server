...
import { sendOtpNotification } from "./notification-delivery.service";
...
export const registerCustomer = async (data: CustomerUser["body"]) => {
  ...
  await sendOtpNotification(customer.id, otp);
...
export const loginClientService = async (phone: string) => {
  const customer = await db.customer.findUnique({
    where: { phone },
